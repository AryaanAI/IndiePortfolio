...
interface SubscriptionContextType {
  ...
  getRemainingPublishedItems: () => number;
}
...
export const SubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [publishedItemsCount, setPublishedItemsCount] = useState(0);
  ...
  useEffect(() => {
    if (user) {
      ...
      // Load published items count
      const workItems = JSON.parse(localStorage.getItem('workItems') || '[]');
      const publishedCount = workItems.filter((item: any) => item.status === 'published').length;
      setPublishedItemsCount(publishedCount);
    }
  }, [user, plans]);
  ...
  const getRemainingPublishedItems = () => {
    if (!currentPlan) return 0;
    if (currentPlan.features.maxPublishedItems === -1) return Infinity;
    return Math.max(0, currentPlan.features.maxPublishedItems - publishedItemsCount);
  };
